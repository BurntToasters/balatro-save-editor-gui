...
// Thin wrapper over the GitHub CLI. Release scripts authenticate through `gh auth login`
// (credentials in gh's keyring); token env vars are stripped so a stale GH_TOKEN can't win.
...
function releaseAssetUploadArgs(uploadUrl, filePath) {
  ...
  const contentType = /\.(asc|txt|json)$/i.test(filePath) ? 'text/plain' : 'application/octet-stream';
  return [
    'api',
    '--method',
    'POST',
...
function runGitHub(args, { input } = {}) {
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT') {
      throw new Error(
        'GitHub CLI is required. Install gh and run `gh auth login` on this release machine. ' +
          'Token environment variables are intentionally ignored; gh must hold credentials in its keyring.',
      );
    }
    throw result.error;
  }
